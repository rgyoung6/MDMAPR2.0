...
shinyAppServer <- function(input, output, session) {
  ...
  popup_data <- reactive({
    ...
    content.output <- paste("<strong><h5>Species:", data$species, "</strong>",
                            ...
                            "<strong><h5>Copy Number Value:", ifelse(is.numeric(data$copyNumber),ifelse(data$copyNumber<=0, 0, exp(data$copyNumber)),0), "</strong>",
  ...
  # Observe will track if any filtering widgets are used. If any filters are implemented the
  # Leaflet map will clear the existing markers and update the markers to relflect the
  ...
  observeEvent(input$Uploaded_SC_submit | input$submit_MDMAPR_file, isolate({
